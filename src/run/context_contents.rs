...
impl RuntimeRef {
    ...
    pub fn is_ref_variant(&self, ctx: &ExecutionContext) -> bool {
        match &ctx.objs.ref_get(*self).variant {
            ValueVariant::Ref(_) | ValueVariant::CharRef(_) | ValueVariant::Ptr(_) => true,
            _ => false,
        }
    }
    ...
    pub fn deref(&self, ast: &AST, ctx: &mut ExecutionContext) -> RuntimeRef {
        ...
        let ret_variant = match variant {
            ValueVariant::CharRef(char_ref) => ValueVariant::ImplicitCharRef(*char_ref),
...
impl Value {
    pub fn to_runtime_ref(self, ctx: &mut ExecutionContext, scope: RuntimeScopeID) -> RuntimeRef {
        RuntimeRef {
            scope,
...
impl RuntimeRef {
    ...
    pub fn dup_in_scope(
        ...
        let new_value = match &value.variant {
            ...
            | ValueVariant::Ref(_)
            | ValueVariant::Ptr(_)
            | ValueVariant::ImplicitCharRef(_)
